...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter ++){
		...
		var oppScore = parseInt(table.rows[row_counter].cells[3].innerHTML);
		...
		if(homeScore > oppScore){
			...
			var oppName = table.rows[row_counter].cells[1].innerHTML;
	...
	document.getElementById('losses').innerHTML = totalLoss;
...
function switchPlayers(playerNum){
	...
	document.getElementById("avg_p_yards").innerHTML = Math.ceil((players[playerNum].pass_yards) / (players[playerNum].games_played));
	document.getElementById("avg_r_yards").innerHTML = Math.ceil((players[playerNum].rushing_yards) / (players[playerNum].games_played));
	document.getElementById("avg_rec_yards").innerHTML = Math.ceil((players[playerNum].receiving_yards) / (players[playerNum].games_played));
}
